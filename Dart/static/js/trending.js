...
const COUNTRY_CODE = 'ID'; // Replace with the desired country code
...
async function fetchPopularMovies() {
  try {
    ...
    return data.results.slice(0, 20); // Return the top 10 popular movies
  } catch (error) {
...
async function fetchRecentReleases() {
  try {
    ...
    const data = await response.json();
    return data.results.slice(0, 20); // Return the top 10 recent releases
  } catch (error) {
...
async function fetchTopRatedMovies() {
  try {
    ...
    const data = await response.json();
    return data.results.slice(0, 20); // Return the top 10 top-rated movies
  } catch (error) {
